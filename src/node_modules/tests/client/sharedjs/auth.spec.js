...
var mock = null;
/**
 * Doesn't return mock itself - instead, it's kind of 'controller' object
 */
function mockAxios() {
  const axios = authRewire.__get__('axios');
  return new AxiosMocker(axios);
}

var promise = null;
function setupWithRespone(status, payload) {
  mock.reset();
  mock.onPost(LOCAL_AUTH_URL).reply(status, payload);
