...
const INITIAL_STATE = {selectedIngredient: {name: '', ndbno: '-1'}, quantity: DEFAULT_QUANTITY};
...
export default class AddMealContainer extends React.Component {
  ...
  buttonClicked() {
    if(this.state.selectedIngredient.name) {
      this.props.onAdd(this.state.selectedIngredient, this.state.quantity);
    }
    this.setState(INITIAL_STATE);
  ...
  quantityChanged(e) {
    ...
    if(newValue) {
      this.state.quantity = parseInt(newValue);
    } else {
      this.state.quantity = DEFAULT_QUANTITY;
    }
      ...
      <Form inline>
        <FormGroup className="s__AddMeal">
          ...
          <InputGroup className="s__foodlog_quantity">
            <FormControl placeholder={DEFAULT_QUANTITY.toString()} onChange={this.quantityChanged}/>
