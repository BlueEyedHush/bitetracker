...
export default class FoodLogContainer extends MealsStoreStateSubscriber(React.Component) {
  ...
  render() {
    return (
      <FoodLog date={this.state.date.toISOString()}
               onDateChanged={dateStr => MealsActions.changeDate(moment(dateStr))}
               meals={this.state.meals}
               onMealAdded={({name, ndbno}) => MealsActions.addMeal(this.state.date.toISOString(), name, ndbno)}
               dataReady={this.state.chartDataReady}
               nutrientValues={this.state.nutrientsTodayCumulative}
               nutrientUnits={this.state.nutrientsTodayCumulativeUnits}
      />
    );
  }
}
