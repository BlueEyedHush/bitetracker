...
class MealsStore {
  ...
  _resetNutrientsData() {
    ...
    this.succcessfulNutrientsFetches = 0;
  ...
  handleNutrientsFetchSuccess({ndbno, nutrientsList}) {
    ...
    this.succcessfulNutrientsFetches += 1;

    const allFetched = this.succcessfulNutrientsFetches + this.failedNutrientFetches;
    if(allFetched >= this.meals.length) {
      if(allFetched > this.meals.length) {
        console.log('Warning: fetched more nutrient info than meals!');
      }

      if(this.succcessfulNutrientsFetches > 0) {
